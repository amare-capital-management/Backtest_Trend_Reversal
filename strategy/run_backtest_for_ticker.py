...
def run_backtest_for_ticker(
    ...
    # Check for NaN values in OHLC columns and log for debugging 
    if data[['Open', 'High', 'Low', 'Close']].isnull().any().any():
        logging.debug(f"NaN values found in OHLC columns for {ticker}:")
        logging.debug(data[['Open', 'High', 'Low', 'Close']].isnull().sum())
        logging.debug("First few rows with Nan:")
        logging.debug(data[data[['Open', 'High', 'Low', 'Close']].isnull().any(axis=1)].head())

    # Remove rows with NaN in OHLC columns to prevent Backtest error 
    original_len = len(data)
    data = data.dropna(subset=['Open', 'High', 'Low', 'Close'])
    if len(data) < original_len:
        logging.debug(f"Dropped {original_len - len(data)} rows with NaN in OHLC columns for {ticker}")

    #Ensure DataFrame is not empty after dropping NaNs
    if data.empty:
        logging.debug(f"DataFrame for {ticker} is empty after dropping NaN values")
        return pd.Series(), pd.DataFrame(), last_day_result
    ...
    bt = Backtest(
        ...
        commission=0.001,
        trade_on_close=True, # Execute trading signals on today's close price or tomorrow at the open
